...
public class ReminderServiceImpl implements Startable {
	...
	public void start() {
		...
		try {
			groupCalendarDataList = calendarService.getGroupCalendars(new String[]{"/platform/users"}, true, username);
			...
			if (!isExistGroupMaintenance){
				...
				initializeCalendarMaintenance(calendarService, cal, nameCalendarMaintenance );
			}	 
			else{
				log.info("Exist already group calendar: "+ nameCalendarMaintenance);
			}
		} catch (Exception e) {
			log.error(e);
		}
	...
	/**
	 * initialize Maintenance with view permission for all users, edit permission for admins
